...
function Testimonial({
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 opacity-[0.03] bg-[radial-gradient(circle_at_20%_20%,#000_0,transparent_40%),radial-gradient(circle_at_80%_60%,#000_0,transparent_35%)]"
        aria-hidden
      />
      <div className="max-w-[900px] relative">
        <div
          className={`absolute -top-10 -left-6 text-[180px] text-black/5 font-serif select-none ${inView ? "scale-100" : "scale-90"}`}
          aria-hidden
        >
          “
        </div>
        <p
          className={`text-[clamp(22px,3.2vw,36px)] leading-[1.5] text-[#1a1a1a] tracking-[-0.04em] mask-gradient transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        >
          {words.map((w, i) => (
            <span
              key={i}
        ...
        <div
          className={`mt-12 flex flex-col ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"} transition-all duration-700`}
