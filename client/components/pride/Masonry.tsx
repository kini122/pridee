...
export default function Masonry({
  ...
  const grid = useMemo(() => {
    ...
    return items.map((child) => {
      ...
      const height = Math.max(80, Math.round((child.height / 2)));
      const y = colHeights[col];

      colHeights[col] += height;

      return { ...child, x, y, w: columnWidth, h: height } as Item;
    });
  }, [columns, items, width]);
  ...
  const handleMouseLeave = (e: any, item: Item) => {
    const selector = `[data-key="${item.id}"]`;
      ...
      {grid.map((item) => {
        return (
          ...
          >
            <div
              className="item-img"
              style={{ backgroundImage: `url(${item.img})`, width: item.w, height: item.h }}
            >
              {colorShiftOnHover && (
                <div
