...
export default function Stats() {
  ...
  return (
    <section id="stats" className="relative min-h-[80vh] py-[120px] px-[40px] snap-start">
      <div
        aria-hidden
      ...
      <div className="max-w-[1200px] mx-auto text-center text-white">
        ...
        <div ref={ref} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[60px] mx-auto max-w-[1200px]">
          {STATS.map((s, i) => (
...
function Stat({ target, suffix, label, start, index }: { target: number; suffix: string; label: string; start: boolean; index: number }) {
  const value = useCountUp(target, 2000, start);
  return (
    <div className="text-center px-5 py-10">
      <div className="mx-auto bg-white/60 h-[2px] w-10 mb-6 origin-center" style={{ width: start ? 40 : 0, transition: "width 0.6s ease-out", transitionDelay: `${index * 150}ms` }} />
      <div className="text-[72px] leading-none font-serif tracking-[-0.04em] mb-4">
        {value.toLocaleString()}
        {suffix}
      </div>
      <div className="text-[18px] text-white/80 max-w-[200px] mx-auto">{label}</div>
    </div>
  );
}
